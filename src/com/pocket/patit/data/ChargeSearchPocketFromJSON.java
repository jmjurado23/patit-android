...
/**
 * {@link ChargeSearchPocketFromJSON}
 * 
 * Clase que pasa la información de los bolsillos buscados a partir de un
 ...
 * @version 1.0
 */
public class ChargeSearchPocketFromJSON {
	...
	int type = 0;			//tipo de búsqueda: 0 user, 1 pocket
	
	public ChargeSearchPocketFromJSON(JSONObject json0, int type) {
	...
	private void chargePockets() {
		...
		try {
				...
				searchArray = json0.getJSONArray(Constants.OBJECTS);
				
				if(type == 0){ //user
					user = searchArray.getJSONObject(0);
					
					//obtenemos los bolsillos del JSON
					pocketsArray = user.getJSONArray(Constants.POCKETS);
				}
				else{
					pocketsArray = searchArray;
				}
