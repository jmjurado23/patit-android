...
/**
 * {@link ChargeSponsoredPocketFromJSON}
 * 
 * Clase que pasa la información de los bolsillos esponsorizados a partir de un
 ...
 * @version 1.0
 */
public class ChargeSponsoredPocketFromJSON {
	...
	List<Pocket> pockets;	//bolsillos
	JSONObject json0;		//json
	
	public ChargeSponsoredPocketFromJSON(JSONObject json0) {
	...
	/**
	 * Método para cargar los elementos a partir del json
	 */
	private void chargePockets() {
		JSONArray commentsArray;
		JSONArray sponsoredArray;
		...
		try {
			...
			sponsoredArray = json0.getJSONArray(Constants.OBJECTS);
			
			for(int i=0;i<sponsoredArray.length();i++){
				...
				sponsor = sponsoredArray.getJSONObject(i);
				pocket = sponsor.getJSONObject(Constants.POCKET);
				...
				System.out.println("CARGA POS "+ pos_votes_p);
				System.out.println("CARGA NEG "+ neg_votes_p);
